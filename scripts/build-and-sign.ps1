...
.PARAMETER AppVersion
    Semantic version baked into the EXE for in-app update checks (e.g. 1.2.3). Default 0.0.0-dev.

.PARAMETER UpdateManifestUrl
    HTTPS URL of latest.json for in-app updates. Empty disables the manifest in the build.
...
$releaseInfoPath = Join-Path $pythonDir "_release_info.py"
$av = $AppVersion.Trim()
if (-not $av) { $av = "0.0.0-dev" }
$mu = $UpdateManifestUrl.Trim()
$appJson = ConvertTo-Json $av
$urlJson = ConvertTo-Json $mu
$releaseBody = @"
# Generated by build-and-sign.ps1. Do not edit.
APP_VERSION = $appJson
UPDATE_MANIFEST_URL = $urlJson
"@
Set-Content -LiteralPath $releaseInfoPath -Value $releaseBody -Encoding utf8
Write-Host "Wrote $releaseInfoPath (APP_VERSION=$av)" -ForegroundColor Gray
